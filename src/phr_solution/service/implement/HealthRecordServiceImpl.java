	...
	public List<RecordResponseDto> getAllRecords() {
		...
		try {
			...
			responseDtos = records.stream()
					.map(record -> new RecordResponseDto(record.getId(), record.getPatientId(), record.getDateOfVisit(), record.getDiagnosis(), record.getTreatment(), new Date()))
		...
		try {
			...
			responseDtos = records.stream()
					...
					.map(filteredRecord -> new RecordResponseDto(filteredRecord.getId(), filteredRecord.getPatientId(), 
							filteredRecord.getDateOfVisit(), filteredRecord.getDiagnosis(), filteredRecord.getTreatment(), new Date()))
					.collect(Collectors.toList());
